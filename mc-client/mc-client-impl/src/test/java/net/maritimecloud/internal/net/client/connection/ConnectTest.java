...
public class ConnectTest extends AbstractClientConnectionTest {
    ...
    public void connectedSlow() throws Exception {
        ...
        c.broadcast(new HelloWorld().setMsg("foo1"));// enqueue before we have actually connected.
        Thread.sleep(50);
        t.send(new Connected().setConnectionId("ABC").setLastReceivedMessageId(0L));
        c.broadcast(new HelloWorld().setMsg("foo2"));
        assertTrue(c.connection().awaitConnected(10, TimeUnit.SECONDS));
        assertTrue(c.connection().isConnected());
        assertEquals("foo1", ((HelloWorld) MessageHelpers.tryRead(t.take(BroadcastPublish.class))).getMsg());
        assertEquals("foo2", ((HelloWorld) MessageHelpers.tryRead(t.take(BroadcastPublish.class))).getMsg());
    }
}
